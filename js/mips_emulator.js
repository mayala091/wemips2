 ...
 * @param  {Object} mipsArgs Arguments to construct the mips emulator.
...
function mipsEmulator(mipsArgs){
    ...
    mipsArgs = _.defaults(mipsArgs, {
        ...
        //TODO: add assembly function here?
   ...
   var stack = new Stack({onChange: mipsArgs.onStackChange, baseAddress: mipsArgs.baseStackAddress});
    ...
    var preservedRegs = [ // these are the registers that are preserved across function calls
        ...
        //'$at', '$k0', '$k1'
        // commented out, need to test and determine where, if any problems exist.
    ];
     ...
     */
        ...
        //removed $ra from this var due to its not being a read only register.
     ...
     */
    var currentLine = 1;
    // populate registers with all the read and write registers and set their initial values to random
    for (var i = 0; i < allRegs.length; i++) {
        registers[allRegs[i]] = createRegister(allRegs[i], i);
        startingRegisters[allRegs[i]] = registers[allRegs[i]];
        if(debug) {
            console.log("register values: ", allRegs[i], " value of i: ", i);
            console.log("register values: ", registers[allRegs[i]]);
        }
    }
    ...
    var ME = {
        ...
        getRegister: function(reg) {
            if (debug) {console.log("getRegister mips_emulator.js: ", JSON.stringify(registers[reg]));}
            if(reg.charAt(0) != '$') reg = '$'+reg;
        ...
        setRegisterVal: function(reg, value, enableCallback) {
            ...
            if(debug) console.log("----> New value: "+ JSON.stringify(ME.getRegister(reg)));
        ...
        getAssembledLine: function(lineNo) {
            var line = mipsCode.code[lineNo];
            var result;

            if (!line || !line.assembledInstruction)
                result = "";
            else
                result = line.assembledInstruction;
    ...
    // Private Methods
    ////////////////////////////////////////////////
   // TODO: determine if this is in the right place
    // should this be here?
    ...
    var assembler = mipsAssembler(instructionExecutor);
    if(debug) {console.log("Call to mipsAssembler from mips_emulator.js line 560");}
    ...
    function mipsLine(line, lineNo){
         ...
         */
        var LINE = {
            // TODO: add an arguemnt for the nameFormatMap from the mips_emulator.js
            /**
            ...
            args: [],
            /**
             * The line's instruction ex: ADD
             * @type {String}
             */
            instruction: null,
            /**
             * The line assembled to its machine code representation.
             * @type {String}
             */
        ...
        if(ar){
            ...
            if (LINE.instruction) {
                ...
                var error = {};
                var namedArgs = {};
                if (!instructionExecutor.parseInstruction(LINE.instruction, LINE.args, namedArgs, error)) {
                    LINE.error = error.message;
                } else {
                    LINE.assembledInstruction = assembler.assembleMipsLine(LINE, namedArgs);
                    if (debug) console.log("set LINE.assembledInstruction to :", LINE.assembledInstruction);
                }
            };
        ...
        //console.log("This is the keys for the 'line' object", Object.keys(LINE));
        return LINE;
    }
