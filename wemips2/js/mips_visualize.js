...
function visualize (){
    ...
    '<g id="ID">' +
        ...
        '<rect class="idecodeObj" x="270" y="250" width="85" height="120" style="fill:none;stroke:lightgrey;stroke-width:2px;" ></rect>' +
            '<text class="idecode" x="276" y="265" font-size="10px" fill="lightgrey" >RR1</text>' +
            '<!--text class="idecode" x="276" y="276" font-size="10px" fill="lightgrey" >Read register 1</text-->' +

        '<text class="idecode" x="322" y="286" font-size="10px" fill="lightgrey" >RD1</text>' +
...
'<!--line class="timing" id="iftime" x1="25" y1="490" x2="145" y2="490" stroke="lightgrey" stroke-width="10px"></line-->' +
    ...
    var elements = [
        ...
        'RR1', 'RR2', 'WR', 'WD', 'RD1', 'RD2', 'regDst', 'jump', 'branch', 'memRead', 'memToReg', 'aluOp0',
        'aluOp1', 'memWrite', 'aluSrc', 'regWrite'
    ];
    ...
    function getElementData(lineName){
        var tempArgs = CurrentLine["args"];

           switch (lineName) {
               case "pc":
                    if (debug) {console.log("pc is: ", pcStart);}
                   return MIPS.numberToBinaryString((CurrentLine["lineNo"]), 8);
               break;

               case "pcAluIn":
                    // Need to track the pc this is hardwired for now.
                   if (debug) {console.log("pcAluInt is: ", parseInt(0, 2).toString(16));}
                    return MIPS.numberToBinaryString((CurrentLine["lineNo"]), 8);
               break;

               case "inst31ToCtrl":
                   if (debug) {console.log("inst31ToCtrl: ", CurrentLine["assembledInstruction"].slice(0, 6));}
                   return  CurrentLine["assembledInstruction"].slice(0,6);
               break;

               case "instToRR1":
                   if (debug) {console.log("instToRR1: ", CurrentLine["assembledInstruction"].slice(7, 12));}
                   return  CurrentLine["assembledInstruction"].slice(7,12);
               break;

               case "inst20ToRR2":
                   if (debug) {console.log("inst20ToRR2: ", CurrentLine["assembledInstruction"].slice(13,18));}
                   return  CurrentLine["assembledInstruction"].slice(13,18);
               break;

               case "inst20ToMux":
                   if (debug) {console.log("inst20ToMux: ", CurrentLine["assembledInstruction"].slice(13,18));}
                   return  CurrentLine["assembledInstruction"].slice(13,18);
               break;

               case "inst15ToMux":
                   if (debug) {console.log("inst15ToMux: ", CurrentLine["assembledInstruction"].slice(19,24));}
                   return  CurrentLine["assembledInstruction"].slice(19,24);
               break;

               case "inst15ToSignExt":
                   if (debug) {console.log("inst15ToSignExt: ", CurrentLine["assembledInstruction"].slice(19,35));}
                   return  CurrentLine["assembledInstruction"].slice(19,35);
               break;

               case "inst5ToAluCtrl":
                   if (debug) {console.log("inst5ToAluCtrl: ", CurrentLine["assembledInstruction"].slice(30,35));}
                   return  CurrentLine["assembledInstruction"].slice(30,35);
               break;
               ...
               default:
            console.log("getElementData: something went horribly wrong");
        }
    }
    ...
    function control (lineName){
        ...
        if (MIPS.binaryStringToNumber(CurrentLine["assembledInstruction"].slice(0, 6)) === 0 ) {
            // R format math
            for (var i = 0; i < elements.length; i++) {
        ...
        } else if (MIPS.binaryStringToNumber(CurrentLine["assembledInstruction"].slice(0, 6)) == 35 ) {
            // LW instruction
            for (var i = 0; i < elements.length; i++) {
                var tempElement = mipsValues[elements[i]];
                switch (tempElement) {
                    case "memRead":
                    case "memToReg":
                    case "aluSrc":
                    ...
                    case "memWrite":
                        ...
                        break;
                    default:
                        console.log("Control op 35: something went horribly wrong");
                }
            }
                }
        else if (MIPS.binaryStringToNumber(CurrentLine["assembledInstruction"].slice(0, 6)) == 43 ) {
            // SW instruction
            for (var i = 0; i < elements.length; i++) {
                var tempElement = mipsValues[elements[i]];
                switch (tempElement) {
                    ...
                    case "regWrite":
                        ...
                        break;
                    default:
                        console.log("Control op 43: something went horribly wrong");
                }
            }
        }
        else if (MIPS.binaryStringToNumber(CurrentLine["assembledInstruction"].slice(0, 6)) == 4 ) {
            // BEQ instruction
            for (var i = 0; i < elements.length; i++) {
                var tempElement = mipsValues[elements[i]];
                switch (tempElement) {
        ...
        else if (MIPS.binaryStringToNumber(CurrentLine["assembledInstruction"].slice(0, 6)) == 2 ) {
            // J instruction
            for (var i = 0; i < elements.length; i++) {
                var tempElement = mipsValues[elements[i]];
                switch (tempElement) {
    ...
    var instructionIn = CurrentLine["text"].slice(0,17);
    ...
    d3.select("#start").append("text")
        ...
        d3.select("#start").append("text")
            .text("Binary: " + CurrentLine["assembledInstruction"].slice(0,35))
            .style("font-size", "12px")
